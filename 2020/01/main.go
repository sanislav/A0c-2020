...
func findThreeSum(input []int, expectedSum int) []int {
	for i := 2; i < len(input); i++ {
		found := findTwoSum(input[i:], expectedSum - input[i])
		if len(found) == 2 {
			return []int{input[i], found[0], found[1]}
		}
	}
...
func main() {
	...
	sort.Ints(input)
	expectedSum := 2020
	two := findTwoSum(input, expectedSum)
	fmt.Println("The two entries summing up to 2020 are ", two[0], two[1])
	fmt.Println("Their multiplication is ", two[0] * two[1])

	three := findThreeSum(input, expectedSum)
	fmt.Println("The three entries summing up to 2020 are ", three[0], three[1], three[2])
	fmt.Println("Their multiplication is ", three[0] * three[1] * three[2])
}